...
STREAM_END_TOKEN = ""
STREAM_FLUSH = (
    False  # if set to True force flush the stream if stream does not ends with "/n"
)
...
async def async_chat(messages):
    """Async chat."""
    try:
        async for resp_chunk in await async_client.chat(
            model="llama3.1", messages=messages, stream=True
        ):
            print(
                resp_chunk["message"]["content"],
                end=STREAM_END_TOKEN,
                flush=STREAM_FLUSH,
            )
        return resp_chunk
...
def chat(messages, stream=False):
    ...
    try:
        if stream:
            resp_chunks = client.chat(
                model="llama3.1", messages=messages, stream=stream
            )
...
async def test_chat(user_message):
    ...
    with PERF_LOGFILE.open("a+") as fw:
        fw.write(
            f"async:True, stream:True, avg_total_duration: {avg_total_duration}s \n"
        )
...
def test_chat_sync_stream(user_message):
    ...
    with PERF_LOGFILE.open("a+") as fw:
        fw.write(
            f"async:False, stream:True, avg_total_duration: {avg_total_duration}s \n"
        )
...
def test_chat_sync(user_message):
    ...
    with PERF_LOGFILE.open("a+") as fw:
        fw.write(
            f"async:False, stream:False, avg_total_duration: {avg_total_duration}s \n"
        )
